...
public class CPU extends SYSTEM {
	...
	// Parameterized CPU Constructor to take program counter and Trace switch as
	// input values
	public static void CPU(int X, int Y) {
		...
		try {
			...
			if (Instruction_Type_T.equals("0")) {
				...
				// Checks whether the first half of the value belongs to Type 0
				// or not
			...
			// Initialize the values before and after the execution of the
			// opcode
	...
	public static void ZERO_HLT() {
		...
		OUTPUT_SPOOLING.OUTPUT_SPOOLING(Job_Id, System_Clock, IO_Clock, LOADER.Page_Fault_Clock,
				LOADER.Segment_Fault_Clock, Output);
		try {
			printtrace();
		} catch (Exception e) {
	...
	public static void printtrace() throws IOException {
		if (Trace_Flag == 1) {
			...
			bw1.write(PC_Hex + "    " + IR_Hex + "   " + BR_Hex + "   " + trace_TOS_bef + "   " + trace_StackTOS_bef
					+ "    " + trace_EA_bef + "    " + trace_MemEA_bef + "   " + trace_TOS_aft + "    "
